...
class CategoryBooksScreen extends StatefulWidget {
  ...
  @override
  State<CategoryBooksScreen> createState() => _BooksGenreScreenState();
}
...
class _BooksGenreScreenState extends State<CategoryBooksScreen> {
  @override
  Widget build(BuildContext context) {
    ...
    void getListGenresUI(BooksGenre booksGenre) {
      for (int i = 0; i < booksGenre.collections.length; i += 2) {
        Row row = Row(children: [
          BooksVerticalContainer(
            onTap: () {
              Navigator.pushNamed(context, '/collection_info_screen',
                  arguments: {'id': booksGenre.collections[i].collection_id});
            },
            name: booksGenre.collections[i].name,
            author: booksGenre.collections[i].author,
            image: booksGenre.collections[i].cover_url,
            type: 'silver',
            percent: 10,
          ),
          const SizedBox(
        ...
        try {
          row.children.add(
            BooksVerticalContainer(
              onTap: () {
                Navigator.pushNamed(context, '/collection_info_screen',
                    arguments: {'id': booksGenre.collections[i + 1].collection_id});
              },
              name: booksGenre.collections[i + 1].name,
              author: booksGenre.collections[i + 1].author,
              image: booksGenre.collections[i + 1].cover_url,
              type: 'silver',
              percent: 10,
            ),
          );
    ...
    storeRepository.getCollectionList(genreName);
    return CustomScaffold(
      ...
      child: BlocBuilder<GenresCubit, GenresState>(
        builder: (context, state) {
          ...
          } else if (state is GenresFailState) {
            return Text('проблем');
          } else {
            getListGenresUI(storeRepository.currentBooksGenre!);
                    ...
                    children: [
                      Padding(
                        ...
                        child: Text(
                          ...
                          style:
                              AppTypography.font20gold.copyWith(fontSize: 24),
                        ),
                      ),
